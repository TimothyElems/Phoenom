defmodule Phoenom.TimelineTest do
  use Phoenom.DataCase

  alias Phoenom.Timeline

  describe "posts" do
    alias Phoenom.Timeline.Post

    import Phoenom.TimelineFixtures

    @invalid_attrs %{body: nil, comments: nil, username: nil, likes_count: nil, repost_count: nil}
    ...
    test "create_post/1 with valid data creates a post" do
      valid_attrs = %{body: "some body", comments: "some comments", username: "some username", likes_count: 42, repost_count: 42}

      assert {:ok, %Post{} = post} = Timeline.create_post(valid_attrs)
      assert post.body == "some body"
      assert post.comments == "some comments"
    ...
    test "update_post/2 with valid data updates the post" do
      post = post_fixture()
      update_attrs = %{body: "some updated body", comments: "some updated comments", username: "some updated username", likes_count: 43, repost_count: 43}

      assert {:ok, %Post{} = post} = Timeline.update_post(post, update_attrs)
      assert post.body == "some updated body"
      assert post.comments == "some updated comments"
